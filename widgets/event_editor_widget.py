...
from editor.ability_data import get_ability_list as _get_ability_list, get_skin_list as _get_skin_list
from editor.boss_data import get_all_bosses as _get_all_bosses
from editor.items_data import get_item_list as _get_item_list
from editor.sprite_registry import get_sprite_options as _get_sprite_options_sr
...
class EventEditorWidget(Widget):
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if cmd == "param_dropdown":
                ...
                opts = self._get_param_options(pk, ct)
                if not opts:
                    opts = [("", f"(sin {pk})")]
    ...
    def _get_param_options(self, pk, ct=None):
        ...
        if pk == "item":
            return _get_item_list()
        if pk == "sprite_id":
            return _get_sprite_options_sr()
        if pk == "nivel":
            return _get_map_list()
        ...
        if pk == "habilidad":
            return _get_ability_list()
        if pk == "skin":
            return _get_skin_list()
        if pk == "demo_id":
            return [("golpe_cabeza", "Golpe de Cabeza")]
        if pk == "boss_id":
            return _get_boss_list()
        if pk == "ventana_id":
            import json, os
            from editor.project import get_current_project
            p = get_current_project()
